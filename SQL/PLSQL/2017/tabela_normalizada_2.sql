...
-----------------------------------------------------------------------------Fim de Operações de Demonstração
...
BEGIN
  ...
  vetor_nomes := vetor(       'Laila',       'Marcos',       'Paulo',        'Leonardo',
                              'Henrique',    'Yasmin',       'Flavia',       'Carol',
                              'Leticia',     'Guilherme',    'João',         'Gabriela',
                              'Marcia',      'Lucas',        'Rafael',       'Matheus',
                              'Raquel',      'Enzo',         'Melissa',      'Thiago');

  vetor_sobrenomes := vetor(  'Oliveira',    'Silva',        'Santos',       'Souza',
                              'Lima',        'Pereira',      'Ferreira',     'Costa',
                              'Costa',       'Rodrigues',    'Almeida',      'Alves',
                              'Carvalho',    'Araújo',       'Ribeiro',      'Smith', 
                              'Schmidt',     'Takanashi',    'Wang',         'Martin');
  ...
  FOR i IN tamanho .. ( tamanho + input_total ) LOOP
    ...
    -- Concatenando os dois nomes
    nome    := CONCAT( CONCAT( ( vetor_nomes( 1 + ABS( MOD( dbms_random.random, vetor_nomes.COUNT ) ) ) ),
                                 ' '),
                      ( vetor_sobrenomes( 1 + ABS( MOD( dbms_random.random, vetor_sobrenomes.COUNT ) ) ) ) );
...
BEGIN
  ...
  produtos  := vetor(     'Computador Dell',    'RAM 8GB',          'HDD 1GB',            'Monitor LED 25.0',
                          'RAM 4GB',            'SSD 124GB',        'Pen Drive 32GB',     'Pen Drive 64GB',
                          'Computador ASUS',    'MacBook Air 13',   'Computador Lenovo',  'Cabo HDMI',
                          'Cabo VGA',           'Computador HPRO',  'Caixa de Som',       'Chassi de Computador');

  pagamento := vetor_int(  3299.99,              299.99,             229.99,               674.99,
                           189.90,               359.99,             39.99,                73.99,
                           2249.99,              4249.99,            2549.99,              29.99,
                           19.99,                5599.99,            569.99,               129.99);
...
SELECT        registro.r_id,        registro.nome,
              compras.id_compra,    compras.nome_produto, 
              compras.id_produto,   compras.total_pago
FROM          registro
INNER JOIN    compras

...
int init_block(ProcessControlBlock **block, size_t pid, char *pname, size_t cycles);
ProcessControlBlock * get_block(size_t pid, char *pname, size_t cycles);
...
int proc_add(Computer *computer)
{
	...
	(computer->pid)++;
	st = init_block(&block, computer->pid, name, cycles);
...
int proc_remove(Computer *computer)
{
	...
	else
	{
		...
		computer->proc_queue->rear = scan->prev;

		if (computer->proc_queue->rear == NULL)
...
int display_queue(ProcessQueue *queue)
{
	...
	if (is_empty(queue)) {
		...
		// PID, Name, Cycles, Time
		printf("\n PID \t Cycles \t Time \t\t\t\t P-Name \n");
		return 0;
	...
	while (scan != NULL)
	{
		// PID, Cycles, Time, Command
		printf("\n %zu \t %6zu \t %s \t %s", scan->p_id, scan->cycles, scan->time, scan->p_name);
...
int display_effective_queue(ProcessQueue *queue)
{
	...
	if (is_empty(queue)) {
		...
		// PID, Effective Cycles, Cycles, Time, Command
		printf("\n PID \t TCycles \t Cycles \t Time \t\t\t\t P-Name \n");
		return 0;
	...
	{
		// PID, Effective Cycles, Cycles, Time, Command
		printf("\n %zu \t %zu \t %6zu \t %s \t %s", scan->p_id, scan->effective_cycles, scan->cycles, scan->time, scan->p_name);
...
int display_block(ProcessControlBlock *block)
{
	if (block == NULL)
		return 1;
	printf("\n PID \t Cycles \t Time \t\t\t\t P-Name \n");
	printf("\n %zu \t %6zu \t %s \t %s", block->p_id, block->cycles, block->time, block->p_name);
